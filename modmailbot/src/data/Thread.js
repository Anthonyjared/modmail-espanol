...
class Thread {
  ...
  async replyToUser(moderator, text, replyAttachments = [], isAnonymous = false) {
    ...
    if (isAnonymous) {
      modUsername = (mainRole ? mainRole.name : 'Ayudante');
      logModUsername = `(Anónimo) (${moderator.user.username}) ${mainRole ? mainRole.name : 'Ayudante'}`;
    } else {
    ...
    try {
      ...
      await this.postSystemMessage(`Error mientras se respondía al usuario: ${e.message}`);
    ...
    if (this.scheduled_close_at) {
      ...
      await this.postSystemMessage(`Cancelado el cierre programado de este hilo debido a la nueva respuesta`);
    }
  ...
  async receiveUserReply(msg) {
    ...
    if (msg.content.trim() === '' && msg.embeds.length) {
      content = '<el mensaje contiene embeds>';
    }
    ...
    if (this.scheduled_close_at) {
      await this.cancelScheduledClose();
      await this.postSystemMessage(`<@!${this.scheduled_close_id}> El hilo que estaba programado para ser cerrado recibió una nueva respuesta. Cancelando...`);
    }

    if (this.alert_id) {
      await this.setAlert(null);
      await this.postSystemMessage(`<@!${this.alert_id}> Nuevo mensaje de ${this.user_name}`);
    }
  }
  ...
  async postToUser(text, file = null) {
    ...
    if (! dmChannel) {
      throw new Error('No se pudieron abrir MDs con este usuario. Éste tiene bloqueado al BOT o tiene su configuración de privacidad demasiado alta.');
    }
  ...
  async postToThreadChannel(...args) {
    try {
      ...
      // Channel not found
      if (e.code === 10003) {
        console.log(`[INFORMACIÓN] Falló al enviar un mensaje al canal de ${this.user_name} porque el canal ya no existe. Autocerrando el hilo...`);
        this.close(true);
      } else {
        throw e;
      }
  ...
  async close(suppressSystemMessage = false, silent = false) {
    if (! suppressSystemMessage) {
      ...
      if (silent) {
        await this.postSystemMessage('Cerrando el hilo silenciosamente...');
      } else {
        await this.postSystemMessage('Cerrando el hilo...');
      }
    }
    ...
    if (channel) {
      console.log(`Eliminando el canal ${this.channel_id}...`);
      await channel.delete('Hilo cerrado');
    }
  }
