...
function getErrorResult(msg = null) {
  return {
    url: `El archivo no pudo ser guardado ${msg ? ': ' + msg : ''}`,
    failed: true
  };
}
...
function downloadAttachment(attachment, tries = 0) {
  return new Promise((resolve, reject) => {
    if (tries > 3) {
      console.error('La descarga del archivo falló luego de tres intentos:', attachment);
      reject('La descarga del archivo falló luego de tres intentos.');
      return;
    }
    ...
    tmp.file((err, filepath, fd, cleanupCallback) => {
      ...
      https.get(attachment.url, (res) => {
        ...
        fs.unlink(filepath);
        console.error('Error descargando el archivo, reintentando.');
...
async function saveDiscordAttachment(attachment) {
  if (attachment.size > 1024 * 1024 * 8) {
    return getErrorResult('El archivo es demasiado largo (máximo 8MB)');
  }
  ...
  if (! inboxGuild.channels.has(attachmentChannelId)) {
    throw new Error('Canal de almacenamiento de archivos no encontrado!');
  }

  const attachmentChannel = inboxGuild.channels.get(attachmentChannelId);
  if (! (attachmentChannel instanceof Eris.TextChannel)) {
    throw new Error('El canal de almacenamiento de archivos debe ser de texto!');
  }
...
async function createDiscordAttachmentMessage(channel, file, tries = 0) {
  ...
  try {
    ...
    if (tries > 3) {
      console.error(`El mensaje del almacenamiento de archivos no pudo ser creado luego de tres intentos: ${e.message}`);
      return;
    }
...
function saveAttachment(attachment) {
  ...
  if (attachmentStorageTypes[config.attachmentStorage]) {
    ...
    throw new Error(`Opción de almacenamiento de archivos desconocida: ${config.attachmentStorage}`);
  }
