...
async function createNewThreadForUser(user, quiet = false, ignoreRequirements = false) {
  ...
  if (existingThread) {
    throw new Error('Se intentó crear un nuevo hilo para este usuario pero ya hay uno existente!');
  }
  ...
  let createdChannel;
  try {
    createdChannel = await utils.getInboxGuild().createChannel(channelName, null, 'Nuevo hilo', newThreadCategoryId);
  } catch (err) {
    console.error(`Error creando un nuevo hilo para ${user.username}#${user.discriminator}!`);
    throw err;
  }
  ...
  if (! quiet) {
    ...
    if (config.mentionRole) {
      await newThread.postNonLogMessage({
        content: `${utils.getInboxMention()}Nuevo hilo (${newThread.user_name})`,
  ...
  infoHeaderItems.push(`Edad de la cuenta: **${accountAge}**`);
  ...
  for (const [guildId, guildData] of userGuildData.entries()) {
    ...
    const headerItems = [
      `Apodo: **${utils.escapeMarkdown(nickname)}**`,
      `Entró **${joinDate}** atrás`
    ];
    ...
    if (guildData.member.voiceState.channelID) {
      ...
      if (voiceChannel) {
        headerItems.push(`Canal de voz: **${utils.escapeMarkdown(voiceChannel.name)}**`);
      }
    }
  ...
  if (userLogCount > 0) {
    infoHeader += `\n\nEste usuario tiene **${userLogCount}** hilos previos. Usa \`${config.prefix}logs\` para verlos.`;
  }
  ...
  if (config.updateNotifications) {
    ...
    if (availableUpdate) {
      await newThread.postNonLogMessage(`📣 Nueva versión del BOT disponible (${availableUpdate})!`);
    }
  }

  // If there were errors sending a response to the user, note that
  if (responseMessageError) {
    await newThread.postSystemMessage(`**NOTA:** No se pudo enviar la autorespuesta al usuario. El error fue: \`${responseMessageError.message}\``);
  }
