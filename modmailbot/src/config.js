...
// Load config file
if (foundConfigFile) {
  console.log(`Cargando configuración de ${foundConfigFile}...`);
  try {
    ...
    throw new Error(`Error leyendo el archivo de configuración! El error fue: ${e.message}`);
  }
}
...
const defaultConfig = {
  ...
  "status": "Envíame un mensaje si tienes dudas / problemas o deseas efectuar un reporte!",
  "responseMessage": "Gracias por tu mensaje! Nuestro equipo te estará responiendo lo más pronto posible.",
  "closeMessage": null,
  ...
  "requiredAccountAge": 12, // In hours
  "accountAgeDeniedMessage": "Tu cuenta de Discord es demasiado nueva para usar el BOT.",

  "requiredTimeOnServer": 60, // In minutes
  "timeOnServerDeniedMessage": "Tu cuenta de Discord es demasiado nueva en el servidor para usar el BOT.",
...
if (loadedEnvValues > 0) {
  console.log(`Cargados ${loadedEnvValues} ${loadedEnvValues === 1 ? 'valor' : 'valores'} de las variables de entorno`);
}
...
for (const [prop, value] of Object.entries(userConfig)) {
  if (! defaultConfig.hasOwnProperty(prop)) {
    throw new Error(`Opción desconocida: ${prop}`);
  }
...
if (finalConfig.smallAttachmentLimit > 1024 * 1024 * 8) {
  ...
  console.warn('[ADVERTENCIA] smallAttachmentLimit limitado a 8MB');
}

// Specific checks
if (finalConfig.attachmentStorage === 'discord' && ! finalConfig.attachmentStorageChannelId) {
  console.error('Opción de configuración \'attachmentStorageChannelId\' es requerida con el almacenamiento de archivos \'discord\'');
  process.exit(1);
}
...
for (const numericOpt of numericOptions) {
  if (finalConfig[numericOpt] != null) {
    ...
    if (Number.isNaN(number)) {
      console.error(`Valor numérico inválido: ${numericOpt}: ${finalConfig[numericOpt]}`);
      process.exit(1);
    }
...
for (const opt of required) {
  if (! finalConfig[opt]) {
    console.error(`Falta una opción de config.json requerida: ${opt}`);
    process.exit(1);
  }
}
