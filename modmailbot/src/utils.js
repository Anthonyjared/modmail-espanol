...
function getInboxGuild() {
  ...
  if (! inboxGuild) throw new BotError('El BOT no está en el servidor de ingreso!');
  return inboxGuild;
}
...
function getMainGuilds() {
  ...
  if (mainGuilds.length !== config.mainGuildId.length) {
    if (config.mainGuildId.length === 1) {
      console.warn(`[ADVERTENCIA] El BOT no entró en el servidor principal!`);
    } else {
      console.warn(`[ADVERTENCIA] El BOT no entró en uno o más servidores!`);
    }
  }
...
/**
 * Retorna el canal de registros asignado, o el por defecto si no hay uno
...
function getLogChannel() {
  ...
  if (! logChannel) {
    throw new BotError('Canal de registros (logChannelId) no se encontró!');
  }

  if (! (logChannel instanceof Eris.TextChannel)) {
    throw new BotError('Asegúrate de que la opciín logChannelId tiene la ID de un canal de texto!');
  }
...
async function formatAttachment(attachment, attachmentUrl) {
  ...
  return `**Archivo adjunto:** ${attachment.filename} (${filesize.toFixed(1)}KB)\n${attachmentUrl}`;
}
