...
module.exports = {
  async start() {
    ...
    bot.once('ready', async () => {
      console.log('Conectado! Esperando que los servidores estén disponibles...');
      await Promise.all([
      ...
      console.log('');
      console.log('Listo! Estoy atendiendo MDs');
      console.log('');
    });
...
function initBaseMessageHandlers() {
  ...
  bot.on('messageUpdate', async (msg, oldMessage) => {
    ...
    const oldContent = oldMessage && oldMessage.content || '**No disponible debido a un reinicio del BOT. Por favor, espere...**';
    ...
    if (msg.channel instanceof Eris.PrivateChannel) {
      ...
      const editMessage = utils.disableLinkPreviews(`**El usuario editó el mensaje:**\n\`De:\` ${oldContent}\n\`A:\` ${newContent}`);
  ...
  bot.on('messageCreate', async msg => {
    ...
    if (mainGuilds.length === 1) {
        content = `${staffMention} BOT mencionado en ${msg.channel.mention} por **${msg.author.username}#${msg.author.discriminator}(${msg.author.id})**: "${msg.cleanContent}"\n\n<https:\/\/discordapp.com\/channels\/${msg.channel.guild.id}\/${msg.channel.id}\/${msg.id}>`;
    } else {
        content = `${staffMention} BOT mencionado en ${msg.channel.mention} (${msg.channel.guild.name}) por **${msg.author.username}#${msg.author.discriminator}(${msg.author.id})**: "${msg.cleanContent}"\n\n<https:\/\/discordapp.com\/channels\/${msg.channel.guild.id}\/${msg.channel.id}\/${msg.id}>`;
    }
...
function initPlugins() {
  ...
  console.log(`Cargados ${plugins.length} plugins (${builtInPlugins.length} plugins incorporados, ${plugins.length - builtInPlugins.length} plugins externos)`);
