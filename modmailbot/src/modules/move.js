...
module.exports = ({ bot, knex, config, commands }) => {
  ...
  commands.addInboxThreadCommand('move', '<category:string$>', async (msg, args, thread) => {
    ...
    if (containsRankings[0][1] === 0) {
      thread.postSystemMessage('Categoría no encontrada');
      return;
    }
    ...
    try {
      ...
      thread.postSystemMessage(`Falló al mover el hilo: ${e.message}`);
      return;
    }
    ...
    if (config.syncPermissionsOnMove) {
      ...
      try {
        ...
        thread.postSystemMessage(`Hilo movido a ${targetCategory.name.toUpperCase()}, pero falló al sincronizar los permisos: ${e.message}`);
        return;
      }
    }

    thread.postSystemMessage(`Hilo movido a ${targetCategory.name.toUpperCase()}`);
  });
};
