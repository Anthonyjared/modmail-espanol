...
module.exports = ({ bot, knex, config, commands }) => {
  const logsCmd = async (msg, args, thread) => {
    ...
    const threadLines = await Promise.all(userThreads.map(async thread => {
      ...
      const formattedDate = moment.utc(thread.created_at).format('MMM Hecho [en] HH:mm [UTC]');
      return `\`${formattedDate}\`: <${logUrl}>`;
    }));

    let message = isPaginated
      ? `**Archivo de registros para <@${userId}>** (página **${page}/${maxPage}**, viendo **${start + 1}-${end}/${totalUserThreads}**):`
      : `**Archivo de registros para <@${userId}>:**`;

    message += `\n${threadLines.join('\n')}`;

    if (isPaginated) {
      message += `\nPara ver más, añade el número de una página al final del comando`;
    }
