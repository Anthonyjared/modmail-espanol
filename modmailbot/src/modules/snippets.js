...
module.exports = ({ bot, knex, config, commands }) => {
  ...
  commands.addInboxServerCommand('snippet', '<trigger> [text$]', async (msg, args, thread) => {
    ...
    if (snippet) {
      if (args.text) {
        ...
        utils.postSystemMessageWithFallback(msg.channel, thread, `Fragmento "${args.trigger}" ya existe! Puedes editarlo o eliminarlo con ${config.prefix}edit_snippet y ${config.prefix}delete_snippet respectivamente.`);
      } else {
        // If the snippet exists and we're NOT trying to create a new one, show info about the existing snippet
        utils.postSystemMessageWithFallback(msg.channel, thread, `\`${config.snippetPrefix}${args.trigger}\` responde con: \`\`\`${utils.disableCodeBlocks(snippet.body)}\`\`\``);
      }
    } else {
      if (args.text) {
        ...
        utils.postSystemMessageWithFallback(msg.channel, thread, `Fragmento "${args.trigger}" creado!`);
      } else {
        // If the snippet doesn't exist and the user isn't trying to create it, inform them how to create it
        utils.postSystemMessageWithFallback(msg.channel, thread, `Fragmento "${args.trigger}" no existe! Puedes crearlo con \`${config.prefix}snippet ${args.trigger} texto\``);
      }
    }
  }, {
    aliases: ['s']
  ...
  commands.addInboxServerCommand('delete_snippet', '<trigger>', async (msg, args, thread) => {
    const snippet = await snippets.get(args.trigger);
    if (! snippet) {
      utils.postSystemMessageWithFallback(msg.channel, thread, `Fragmento "${args.trigger}" no existe!`);
      return;
    }

    await snippets.del(args.trigger);
    utils.postSystemMessageWithFallback(msg.channel, thread, `Fragmento "${args.trigger}" eliminado!`);
  }, {
    aliases: ['ds']
  });
  ...
  commands.addInboxServerCommand('edit_snippet', '<trigger> [text$]', async (msg, args, thread) => {
    ...
    utils.postSystemMessageWithFallback(msg.channel, thread, `Fragmento "${args.trigger}" editado!`);
  }, {
    aliases: ['es']
  });
  ...
  commands.addInboxServerCommand('snippets', [], async (msg, args, thread) => {
    ...
    utils.postSystemMessageWithFallback(msg.channel, thread, `Fragmentos disponibles (prefijo: ${config.snippetPrefix}):\n${triggers.join(', ')}`);
  });
};
