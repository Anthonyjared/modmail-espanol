...
async function serveLogs(res, pathParts) {
  ...
  const lines = threadMessages.map(message => {
    ...
    if (message.message_type === THREAD_MESSAGE_TYPE.SYSTEM) {
      ...
      line += `[DEL USUARIO] ${message.user_name}: ${message.body}`;
    } else if (message.message_type === THREAD_MESSAGE_TYPE.TO_USER) {
      line += `[AL USUARIO] ${message.user_name}: ${message.body}`;
    } else {
...
module.exports = () => {
  ...
  server.on('error', err => {
    console.log('[ADVERTENCIA] Error en el servidor web:', err.message);
  });
