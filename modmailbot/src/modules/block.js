...
module.exports = ({ bot, knex, config, commands }) => {
  async function removeExpiredBlocks() {
    ...
    for (const userId of expiredBlocks) {
      ...
      logChannel.createMessage(`El bloqueo de <@!${userId}> (ID: \`${userId}\`) expiró.`);
    }
  }
  ...
  const blockCmd = async (msg, args, thread) => {
    ...
    if (isBlocked) {
      msg.channel.createMessage('El usuario ya está bloqueado.');
      return;
    }
    ...
    if (expiresAt) {
      ...
      msg.channel.createMessage(`Bloqueado <@${userIdToBlock}> (ID: \`${userIdToBlock}\`) del BOT durante ${humanized}`);
    } else {
      msg.channel.createMessage(`Bloqueado <@${userIdToBlock}> (ID: \`${userIdToBlock}\`) del BOT con tiempo indefinido.`);
    }
  };
  ...
  const unblockCmd = async (msg, args, thread) => {
    ...
    if (! isBlocked) {
      msg.channel.createMessage('El usuario no está bloqueado');
      return;
    }
